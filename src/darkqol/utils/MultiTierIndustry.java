...
public abstract class MultiTierIndustry extends AbstractSubmarketIndustry {
    ...
    private int getCurrentTier() {
        // The built industry's own id matches the sub-industry key for its tier,
        // so this reflects the actually-built hub level (not just the max in the map).
        IndustryInfo info = industriesMap.get(getId());
        return info != null ? info.getTier() : 0;
    }
